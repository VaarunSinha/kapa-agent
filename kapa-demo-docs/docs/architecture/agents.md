...
The style guide used by the Writer Agent is derived from existing documentation through the `extract_style` function, located in `kapa_demo_backend/agent/agents/style.py`. This function utilizes the GPT-4o model to extract style attributes from sample documentation content, including tone, sentence style, structure, and formatting rules. The output is compiled into a `style.md` file, ensuring consistency across all documentation generated by the Writer Agent. This process provides a systematic approach to maintaining a coherent writing style, making it easier for contributors to adhere to established guidelines.

Additionally, we utilize structured outputs with the GPT-4o model for deterministic output, which is processable.
