...
It receives the issue (title, description, research goal) and repository context. The system gathers relevant documentation and code content and passes it to the next steps.
...
#### Style Guide Derivation and Maintenance

The style guide is derived from a combination of existing documentation standards and external guidelines, ensuring it reflects current best practices. It is updated periodically based on user feedback, changes in project requirements, and emerging documentation trends. The Writer Agent and Fix Assistant reference this guide to ensure all content adheres to the established style, providing a consistent voice and formatting throughout the documentation. Additionally, the style guide is generated by giving excerpts of the related documentation (which is selected by the researcher) and similar language and style is used in the writer.
