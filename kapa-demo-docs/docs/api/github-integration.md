...
The backend uses the GitHub API under the hood. Once the app is installed on a repository or organization, the backend uses the app's credentials to perform these actions in the context of the installation.

## Typical workflow

1. A user triggers a documentation task (e.g., from the frontend) with a GitHub issue URL.
2. The backend uses the app to read the issue (title, body, labels) and to determine the repository and branch.
3. Repository content is fetched as needed by the research agent.
4. After the writer agent produces the draft, the backend creates a new branch, commits the doc changes, and opens a pull request.
5. Optionally, the backend posts a comment on the original issue with a link to the PR.

All of this is done with the GitHub App's permissions; no user OAuth is required for the agent to operate once the app is installed.
...
The backend can be configured to accept GitHub webhooks (e.g., for "issue labeled" or "issue opened" events) to start a documentation task automatically when a label like `documentation-gap` is added. Webhook handling is optional; the same pipeline can be triggered only via the API from the frontend.
